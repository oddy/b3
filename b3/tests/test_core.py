...
# --------------------------------------------------------------------------------------------------
def test_enc_bytes():
    assert encode_item_joined(None, B3_BYTES, b"\xbe\xef") == SBytes("08 02 be ef")
def test_enc_bytes_z():
    assert encode_item_joined(None, B3_BYTES, b"") == SBytes("00")

def test_dec_bytes():
    assert decode_item_type_value(SBytes("08 02 be ef")) == (B3_BYTES, b"\xbe\xef")
def test_dec_bytes_z():
    assert decode_item_type_value(SBytes("00")) == (B3_BYTES, b"")
...
TEST_UNISTRS = (
    ...
    ("✈✉🚀🚸🚼🚽", SBytes("18 16 e2 9c 88 e2 9c 89 f0 9f 9a 80 f0 9f 9a b8 f0 9f 9a bc f0 9f 9a bd")),  # SMP
    ("", SBytes("10")),  # only the header, no data
)

@pytest.mark.parametrize("tstr,tbytes", TEST_UNISTRS)
def test_enc_utf8(tstr, tbytes):
    assert encode_item_joined(None, B3_UTF8, tstr) == tbytes

@pytest.mark.parametrize("tstr,tbytes", TEST_UNISTRS)
def test_dec_utf8(tstr, tbytes):
    assert decode_item_type_value(tbytes) == (B3_UTF8, tstr)
...
TEST_NUM_VALUES = (
    ("uvarint_0", B3_UVARINT, 0, SBytes("30")),
    ("svarint_0", B3_SVARINT, 0, SBytes("40")),
    ("u64_0", B3_U64, 0, SBytes("50")),
    ("s64_0", B3_S64, 0, SBytes("60")),
    ("f64_0", B3_FLOAT64, 0.0, SBytes("70")),
    ("complex_0", B3_COMPLEX, 0j, SBytes("f0 10")),
    ("uvarint", B3_UVARINT, 123456789, SBytes("38 04 95 9a ef 3a")),
    ("svarint", B3_SVARINT, -123456789, SBytes("48 04 a9 b4 de 75")),
    ...
    ("complex", B3_COMPLEX, 13.37+42.42j, SBytes("f8 10 10 3d 0a d7 a3 70 bd 2a 40 f6 28 5c 8f c2 35 45 40")),
)
...
TEST_DECSCHED_VALUES = (
    ("decimal_0", B3_DECIMAL, 0.0, SBytes("80")),
    ("decimal", B3_DECIMAL, decimal.Decimal("12345.6789"), SBytes("88 05 24 95 9a ef 3a")),
    ("sched_0", B3_SCHED, datetime.datetime(1, 1, 1), SBytes("90")),
    ("sched", B3_SCHED, datetime.datetime(2022, 4, 4, 16, 45, 43, 2718), SBytes("98 0a c2 cc 1f 04 04 10 2d 2b 9e 15")),
)
