...
def test_enc_header_all():
    assert encode_item(key=u"foo", data_type=555, value=b"\xbe\xef") == \
        (SBytes("fa ab 04 03 66 6f 6f 02"), SBytes("be ef"))
...
def test_dec_header_all():
    assert decode_header(SBytes("fa ab 04 03 66 6f 6f dc 0b"), 0) == \
        (u"foo", 555, True, False, 1500, 9)
...
def test_dec_header_keys():
    ...
    assert decode_header(SBytes("02 0c d0 92 d0 b8 d0 b0 d0 b3 d1 80 d0 b0"), 0)  == (u"Виагра", 0, False,  False, 0, 14)
